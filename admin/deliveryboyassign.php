...
if ($delid != ""){
    $ddate=date("m/d/Y");
    $upsql1 = "SELECT * FROM order_master where id={$delid}";
    $res11=mysqli_query($conn,$upsql1);
    $row11=mysqli_fetch_assoc($res11);
    $ses_id=$row11['session_id'];
    $sdq="UPDATE cart SET status='delivered' WHERE session_id='$ses_id'";
    $resdf=mysqli_query($conn,$sdq);
    
    $upsql = "UPDATE order_master set order_status='Delivered',delivery_date='$ddate' where id={$delid}";            
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-7 align-self-center">
                        <h4 class="page-title text-truncate text-dark font-weight-medium mb-1">Product Details</h4>
                                    ...
                                    <table id="zero_config" class="table table-striped table-bordered no-wrap">
                                        ...
                                        <tbody>
                                              <?php
                                                ...
                                                $sqlfetch = "SELECT * FROM order_master where order_status='dispatch'";
                                            ...
                                            <tr>
                                                ...
                                                <td><?php if($row['order_status']=='dispatch'){?>
                                                <a href="?delid=<?php print $row['id']; ?>" class="btn btn-sm bg-success" style="color:white;" onclick ="return confirm('Are you sure to Change the product status To Delivered ?')"><i class="fa fa-close" aria-hidden="true"></i> Out Of Delivery</a>
                                                <?php } else {?>
                                                <a href="#" class="btn btn-sm bg-danger" style="color:white;min-width: 82px;"><i class="fa fa-check" aria-hidden="true"></i>Delivered</a>
                                                <?php } ?></td>
